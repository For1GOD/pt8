...
MONSTER_WIDTH = 32
MONSTER_HEIGHT = 32
MONSTER_COLOR = '#21100F'
BASE_DIR = os.path.dirname(__file__)

ANIMATION_MONSTER_HORISONTAL = [
...
class Monster(sprite.Sprite):
    def __init__(self, x, y, letf, up, max_length_left, max_length_up):
        super().__init__()
        ...
        self.y_vel = up
        bolt_anim = list()
        for anim in ANIMATION_MONSTER_HORISONTAL:
        ...
        def update(self, platforms):
            ...
            if abs(self.start_x - self.rect.x) > self.max_lendth_left:
                self.x_vel = -self.x_vel
            elif  abs(self.start_y - self.rect.y) > self.max_lendth_up:
